...
# ============================================================
# FONCTIONS DE TRANSFORMATION DE LA CIBLE
# IMPORTANT : elles doivent être définies AVANT joblib.load()
# ============================================================
...
MODEL_PATH = "model/modele_final_SimpleImputer_TargetEncoding_log.pkl"
...
# ============================================================
# EXTRACTION DU PRÉPROCESSEUR ET DU RANDOM FOREST
# ============================================================

try:

    # Le nom réel dans le modèle est "preprocessing"
    preprocessor = model.named_steps["preprocessing"]

    # Le nom réel est "regressor"
    target_regressor = model.named_steps["regressor"]

    # Random Forest contenu dans le TransformedTargetRegressor
    rf_model = getattr(
        target_regressor,
        "regressor_",
        target_regressor.regressor
...
try:
    ...
    print(
        "AVERTISSEMENT : SHAP n'a pas pu être initialisé :",
        e
    )
...
def create_dataframe(data: PredictionInput):

    row = {

        "age_vehicule_annees": data.age_vehicule_annees,
        "duree_garantie_jours": data.duree_garantie_jours,
        "places": data.places,
        "puissance": data.puissance,
        "valeur_neuve": data.valeur_neuve,
        "valeur_venale": data.valeur_venale,
        "cout_total_sinistres": data.cout_total_sinistres,

        "categorie_mère": data.categorie_mère,
...
def calculate_shap(df):
    ...
    if explainer is None:
        ...
        return {
            "available": False,
            "message": "Explainer SHAP indisponible.",
            "image_url": None,
            "contributions": [],
        }
    ...
    try:
        ...
        rf_prediction_log = float(
            rf_model.predict(X_processed)[0]
        )
        ...
        # Gestion des différentes dimensions
        if shap_values.ndim == 3:
        ...
        return {
            ...
            "expected_value_log": expected_value,

            "prediction_log": rf_prediction_log,
...
def model_info():
    ...
    return {
        ...
        "preprocessing": [
            ...
            "SimpleImputer médiane pour les variables numériques",

            "SimpleImputer most_frequent pour les variables catégorielles",
